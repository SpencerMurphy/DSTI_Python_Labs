...
class ClassifPredictionModel:
	def __init__(self, path_model, path_classes,detection_window = None):
		...
		Parameters:
		-----------
		- path_model: str, path to the model file generated after training
		- path_classes: str, path to the file containing the list of classes name
		- detection_window : list with top-left x,top-left y and bottom-right x,bottom-right y coordonates 
		of the image window used for detection 
		Attributes:
		-----------
		...
		self.detection_rows =  slice(detection_window[1],detection_window[3])
		self.detection_columns =  slice(detection_window[0],detection_window[2])
	...
	def predict_and_identify(self, image, threshold=0.5):
		...
		cv2.rectangle(image, self.detection_upleft, self.detection_downright, (0, 0, 255), 0)
...
if __name__ == "__main__":
	classif = ClassifPredictionModel(PATH_MODEL, PATH_CLASSES,(300,50,550,300))
	...
	while True:
		...
		# Predictions
		pred_class_idx, pred_proba_value, text  = classif.predict_and_identify(frame, 0.5)
		print(pred_class_idx, pred_proba_value, text)
